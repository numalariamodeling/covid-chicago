...
def plot(adf,age_group, first_day, filename=None) :

    fig = plt.figure(figsize=(12,6))

    plotchannels = [ '%s_%s' % (x, age_group) for x in [
        'susceptible', 'exposed', 'asymptomatic', 'symptomatic_mild',
        'hospitalized', 'critical', 'deaths',
        'new_detected', 'new_detected_hospitalized', 'new_critical', 'new_deaths', 'new_detected_deaths']]
...
if __name__ == '__main__' :
    ...
    params = pd.read_csv(os.path.join(sim_output_path, 'sampled_parameters.csv'))
    first_day = datetime.strptime(params['startdate'].unique()[0], '%Y-%m-%d')
    ...
    for age_group in suffix_names :
        ...
        adf = df[cols]
        adf = calculate_incidence_by_age(adf, age_group,
                                         output_filename=os.path.join(sim_output_path, 'trajectoriesDat_withIncidence_%s.csv' % age_group))
        adf['infections_cumul_%s' % age_group] = adf['asymp_cumul_%s' % age_group] + adf['symp_mild_cumul_%s' % age_group] + adf['symp_severe_cumul_%s' % age_group]
        ...
        plot(adf, age_group, first_day, 'plot_withIncidence_%s' % age_group)
