...
if(runinBatchMode){
  ...
  length(cmd_agrs)
  area_nr <- cmd_agrs[length(cmd_agrs)]
...
print(area_nr)
if(area_nr==1)restoreRegion="northcentral"
if(area_nr==2)restoreRegion="northeast"
if(area_nr==3)restoreRegion="central"
if(area_nr==4)restoreRegion="southern"
if(area_nr==5)restoreRegion="All"
print(restoreRegion)
...
colnames(tempdat)[colnames(tempdat)== paste0( "infected_cumul_",restoreRegion)]  = "infected_cumul"
colnames(tempdat)
...
tempdat <- tempdat %>% 
  ...
  dplyr::summarize(new_infections =mean(new_infections),
                   infected_cumul = mean(infected_cumul))
...
for (scen in unique(tempdat$reopening_multiplier_4)) {
  ...
  Rt_tempdat  <- res$R %>% mutate(region = restoreRegion, weekwindow=weekwindow )
  ...
  SI_tempdat  <- res$SI.Moments %>% mutate(region = restoreRegion, weekwindow=weekwindow )
...
save(Rt_tempdat_All, file=file.path(Rt_dir, paste0(restoreRegion,"_Rt_aggregated_scen_num.Rdata")))
