...
ggsave(paste0(channel,".pdf"),
       plot = pplot,
       path = file.path(wdir,  "parameters","vaccinations"), width = 18, height = 10, device = "pdf"
)
}
return(pplot)
}
...
pplot <- ggplot(data=lmdatAggr)+
  ...
  geom_point(data=datAggr,aes(x=date, y=persons_first_vaccinated),col='black',size=0.8)+
  theme_cowplot()+
  background_grid()+
  scale_y_continuous(labels=comma)+
...
ggsave("IL_vaccination_scale_up.pdf",
       plot = pplot,
       path = file.path(wdir,  "parameters","vaccinations"), width = 8, height = 6, device = "pdf"
)