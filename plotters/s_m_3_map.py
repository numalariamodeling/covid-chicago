...
if __name__ == '__main__' :
    ...
    vmin, vmax = 0, np.max(df['s_m_3'])
    norm = colors.Normalize(vmin=vmin,
                            vmax=vmax)
    ax = fig.add_subplot(1,1,1)
    df_shp = pd.merge(left=ems_shp, right=df, left_on='REGION', right_on='EMS')
    df_shp.plot(column='s_m_3', ax=ax, legend=False, cmap='Blues', norm=norm, edgecolor='0.8',
                linewidth=0.8)
    ...
    ax.set_title('social_multiplier_3', fontsize=20)
    ax.set_xticks([])
    ax.set_yticks([])
    ax.axis('off')

    plt.savefig(os.path.join(r'C:\Users\bluen\Desktop\figures', 's_m_3_map.pdf'), format='PDF')
    plt.close(fig)