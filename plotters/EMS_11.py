...
def generateParameterSamples(samples, pop, first_day):
    ...
    df['cfr'] = np.random.uniform(0.058, 0.092, samples)  # np.random.uniform(0.008, 0.022, samples)
        ...
        for i in Kivalues:
            ...
            if not Location == 'Local':
                ...
                data_cfg = data_cfg.replace('trajectories',
                                            './_temp/' + exp_name + '/trajectories/trajectories_scen' + str(scen_num))
            else:
...
if __name__ == '__main__':
    ...
    region = 'EMS_8'
    exp_name = today.strftime("%Y%m%d") + '_%s_mr_run2.1' % region

    # Selected SEIR model
    emodlname = 'extendedmodel_cobey.emodl'

    # Generate folders and copy required files
    temp_dir, temp_exp_dir, trajectories_dir, sim_output_path, plot_path = makeExperimentFolder(exp_name, emodl_dir,
                                                                                                emodlname, cfg_dir)
    ...
    combineTrajectories(Nscenarios=nscen, trajectories_dir=trajectories_dir, temp_exp_dir=temp_exp_dir,
                        deleteFiles=False)
