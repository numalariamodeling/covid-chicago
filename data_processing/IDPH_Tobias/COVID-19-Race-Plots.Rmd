...
tests_per_zip <- data.frame(table(cases$patient_home_zip))
colnames(tests_per_zip) <- c("zip","n_tests")
z <- data.frame(table(cases$patient_home_zip,cases$race_eth=="White"))
...
tests_per_zip <- data.frame(table(cases$patient_home_zip))
colnames(tests_per_zip) <- c("zip","n_tests")
z <- data.frame(table(cases$patient_home_zip,cases$race_eth=="Hispanic-Latino"))
...
tests_per_zip <- data.frame(table(cases$patient_home_zip))
colnames(tests_per_zip) <- c("zip","n_tests")
z <- data.frame(table(cases$patient_home_zip,cases$race_eth=="Black"))
...
ems_race <- data.frame(table(cases$EMS,cases$race_eth))
...
```{r}
a <- as.data.frame(table(cases$EMS,cases$race_eth))
...
cases$RR <- NA
cases$RR[cases$EMS %in% c("1","2")] <- "North-Central"
cases$RR[cases$EMS %in% c("3","6")] <- "Central"
cases$RR[cases$EMS %in% c("4","5")] <- "Southern"
cases$RR[cases$EMS %in% c("7","8","9","10","11")] <- "Northeast"

cases$RR <- as.factor(cases$RR)
...
cases$race_for_cfr_plot <- "Other/NA"
cases$race_for_cfr_plot[cases$race_eth=="White"] <- "White"
cases$race_for_cfr_plot[cases$race_eth=="Black"] <- "Black"
cases$race_for_cfr_plot[cases$race_eth=="Hispanic-Latino"] <- "Latinx"

cases$race_for_cfr_plot <- factor(cases$race_for_cfr_plot)

for(i in 1:length(levels(cases$RR))) 
  {
  
  for (j in 1:length(levels(cases$race_for_cfr_plot)))
  {
    re <- levels(cases$race_for_cfr_plot)[j]
    region <- levels(cases$RR)[i]
    
    a <- subset(cases, cases$RR == region) # Just region
...
ggplot(data=restore_cfr, aes(x=races,y=fs,fill=races)) + 
  ...
  ggtitle("CFR by Race and Restore Region", "All Ages") +
...
ggplot(data=restore_cfr, aes(x=races,y=fs,fill=races)) + 
  ...
  ggtitle("CFR by Race and Restore Region", "All Ages") +
  labs(fill="Race/Ethnicity") + xlab("") + ylab("(# Deaths / # Cases)") + 
  geom_text(aes(label=paste(format(gs,big.mark = ','),"Cases"),y=fs), size=4, stat="identity", position=position_dodge(0.9),vjust=-0.4) +
  geom_text(aes(label=paste(format(ns,big.mark = ','),"Deaths"),y=fs), size=4, stat="identity", position=position_dodge(0.9),vjust=-4) +ylim(0,0.12)
ggsave(paste(tag,"CFR_byRace_byRestoreRegion",extension,sep="_"),height=10,width=10)


cases_under70 <- subset(cases, !(cases$age_group %in% c("Under 21","21-30","71-80","Over 80")) & cases$age_group != "")
...
for(i in 1:length(levels(cases_under70$RR))) 
  {
  
  for (j in 1:length(levels(cases_under70$race_for_cfr_plot)))
  {
    re <- levels(cases_under70$race_for_cfr_plot)[j]
    region <- levels(cases_under70$RR)[i]
    
    a <- subset(cases_under70, cases_under70$RR == region) # Just region
...
ggplot(data=restore_cfr, aes(x=races,y=fs,fill=races)) + 
  ...
  facet_wrap(~factor(rrs), scales="free_y") +
  theme(axis.text.x = element_blank(), axis.ticks = element_blank()) +
  ggtitle("CFR by Race and Restore Region", "Age 31-70") +
  labs(fill="Race/Ethnicity") + xlab("") + ylab("(# Deaths / # Cases)")
ggsave(paste(tag,"CFR_byRace_byRestoreRegion_30to70_free_y",extension,sep="_"),height=10,width=10)
...
ggplot(data=restore_cfr, aes(x=races,y=fs,fill=races)) + 
  ...
  geom_text(aes(label=paste(format(ns,big.mark = ','),"Deaths"),y=fs), size=4, stat="identity", position=position_dodge(0.9),vjust=-2) +ylim(0,0.06)
ggsave(paste(tag,"CFR_byRace_byRestoreRegion_30to70",extension,sep="_"),height=10,width=10)


cases_june1 <- subset(cases_under70, cases_under70$specimen_collection >= "2020-06-01")
...
for(i in 1:length(levels(cases_june1$RR))) 
  {
  
  for (j in 1:length(levels(cases_june1$race_for_cfr_plot)))
  {
    re <- levels(cases_june1$race_for_cfr_plot)[j]
    region <- levels(cases_june1$RR)[i]
    
    a <- subset(cases_june1, cases_june1$RR == region) # Just region
...
ggplot(data=restore_cfr_june1, aes(x=races,y=fs,fill=races, label = ns)) + 
  ...
  ggtitle("CFR by Race and Restore Region", "Age ≤ 70, since June 1") +
    labs(fill="Race/Ethnicity") + xlab("") + ylab("(# Deaths / # Cases)")
ggsave(paste(tag,"CFR-sinceJune01-_byRace_byRestoreRegion_under70_free_y",extension,sep="_"),height=10,width=10)
...
ggplot(data=restore_cfr_june1, aes(x=races,y=fs,fill=races)) + geom_bar(stat='identity',position='dodge') + 
  ...
  ggtitle("CFR by Race and Restore Region", "Age ≤ 70, since June 1") +
  labs(fill="Race/Ethnicity") + xlab("") + ylab("(# Deaths / # Cases)")
ggsave(paste(tag,"CFR-sinceJune01-_byRace_byRestoreRegion_under70",extension,sep="_"),height=10,width=10)
```
...
case_admit$admitted_to_hospital[which(case_admit$admission_date!="")] <- "Yes"
case_admit$admitted_to_hospital[which(case_admit$admitted_to_hospital=="")] <- "Missing"

case_admit_race <- data.frame(table(case_admit$admitted_to_hospital,case_admit$race_eth))
colnames(case_admit_race) <- c("admit_status","race_eth","n")
case_admit_race$admit_status <- factor(case_admit_race$admit_status,levels=c("Yes","No","Missing","Unknown"))

ggplot(case_admit_race, aes(x=race_eth,y=n, fill=admit_status)) + geom_bar(stat='identity', position='fill') +
...
ggplot(case_admit_race, aes(x=race_eth,y=n, fill=admit_status)) + geom_bar(stat='identity', position='dodge') +
...
case_admit_ems <- data.frame(table(case_admit$admitted_to_hospital,case_admit$EMS))
colnames(case_admit_ems) <- c("admit_status","ems","n")
case_admit_ems$admit_status <- factor(case_admit_ems$admit_status,levels=c("Yes","No","Missing","Unknown"))
case_admit_ems$ems <- factor(case_admit_ems$ems, levels = c("4","5","3","6","1","2","7","8","9","10","11"))

ggplot(case_admit_ems, aes(x=ems,y=n, fill=admit_status)) + geom_bar(stat='identity', position='fill') +
...
ggplot(case_admit_ems, aes(x=ems,y=n, fill=admit_status)) + geom_bar(stat='identity', position='dodge') +
...
```{r}
print(paste(nrow(cases)-nrow(IL_patients),"cases with zip codes that do not match ZCTA",sep=" "))
excluded_by_zip <- subset(cases,!(cases$id %in% IL_patients$id))
excluded_by_zip$exclusion <- NA
excluded_by_zip$exclusion[which(excluded_by_zip$patient_home_zip < 60002 | excluded_by_zip$patient_home_zip > 62999)] <- "Out of State"
excluded_by_zip$exclusion[which(excluded_by_zip$patient_home_zip >= 60002 & excluded_by_zip$patient_home_zip <= 62999)] <- "Non-Standard"

kable(table(excluded_by_zip$exclusion,useNA = 'ifany'),col.names = c("Reason Excluded", "# cases"))


kable(table(excluded_by_zip$race_eth,useNA = 'ifany'),col.names = c("Race/Ethnicity", "# cases excluded from Zip Analysis"))

kable(table(excluded_by_zip$age_group,useNA = 'ifany'),col.names = c("Age Group", "# cases excluded from Zip Analysis"))

# Crestwood (real zip excluded)

crestwood <- subset(cases,cases$patient_home_zip==60418)

kable(table(crestwood$race_eth,useNA = 'ifany'),col.names = c("Race/Ethnicity", "# cases in 60418"))

kable(table(crestwood$age_group,useNA = 'ifany'),col.names = c("Age Group", "# cases in 60418"))


```


Create .csv of cases and deaths by date tested x race

```{r}

tested$race_for_cfr_plot <- "All Others"
...
tested$test_race_label <- paste(tested$race_for_cfr_plot,tested$specimen_collection,sep="_")
tested$test_race_label <- as.factor(tested$test_race_label)

tested$death_race_label <-  paste(tested$race_for_cfr_plot,tested$deceased_date,sep="_")
tested$death_race_label <- as.factor(tested$death_race_label)

daily_cases_deaths_31to70 <- subset(tested,!(tested$age_group %in% c("Under 21","21-30","71-80","Over 80")) & tested$age_group!="" )
daily_cases_deaths_31to70 <- daily_cases_deaths_31to70[,c("id","race_for_cfr_plot","specimen_collection","deceased_date","test_race_label","death_race_label")]

 # reese_cases <- aggregate(reese$id,by=list(reese$test_race_label),FUN=length)
 # colnames(reese_cases) <- c("date_race_label","n_cases")
 # reese_deaths <- aggregate(reese$deceased_date!="",by=list(reese$death_race_label),FUN=sum)
 # colnames(reese_deaths) <- c("date_race_label","n_deaths")
 # 
 # reese_long <- merge(reese,reese_cases, by.x="test_race_label",by.y="date_race_label")
 # reese_long <- merge(reese_long,reese_deaths, by.x="death_race_label",by.y="date_race_label")
 # 
 # write.csv(reese_long,paste(pull_date,"Daily Cases and Deaths by Race - long.csv",sep=" - "))
 # 
 # reese_test <- reese_long[,c("test_race_label","race_for_cfr_plot","specimen_collection","n_cases","n_deaths")]
 # reese_test <- distinct( reese_test)
 # reese_test <-  reese_test[,c(2:5)]
 # colnames(reese_test) <- c("race","specimen_collection","n_cases","n_deaths")
 # 
 # 
 # 
 # write.csv(reese_test,paste(pull_date,"Daily Cases and Deaths by Race (test Date).csv",sep=" - "))

#### Trying another way

test_dates <- unique(paste(daily_cases_deaths_31to70$specimen_collection))[order(unique(paste(daily_cases_deaths_31to70$specimen_collection)))]
death_dates <- unique(paste(daily_cases_deaths_31to70$deceased_date[daily_cases_deaths_31to70$deceased_date!=""]))[order(unique(paste(daily_cases_deaths_31to70$deceased_date[daily_cases_deaths_31to70$deceased_date!=""])))]

dates <- unique(c(test_dates,death_dates))[order(unique(c(test_dates,death_dates)))]
dates <- dates[date!="NA"]
...
{
   for (r in levels(daily_cases_deaths_31to70$race_for_cfr_plot))
   {
      sub <- subset(daily_cases_deaths_31to70,paste(daily_cases_deaths_31to70$race_for_cfr_plot) == r)
      
      c <- nrow(sub[sub$specimen_collection == dates[i],])
      d <- nrow(sub[paste(sub$deceased_date) == dates[i],])
...
daily_cases_deaths_31to70_summary <- data.frame(race = rs, date = ds, n_cases = nc, n_deaths = nd)
write.csv(daily_cases_deaths_31to70_summary,paste(pull_date,"Daily Cases and Deaths by Race (test date).csv",sep=" - "))
...
for(i in 1:length(dates))
{
   ...
   {
      sub <- subset(daily_cases_deaths_31to70,paste(daily_cases_deaths_31to70$race_for_cfr_plot) == r)
      sub <- subset(sub,sub$specimen_collection==dates[i])
...
daily_cases_deaths_31to70_summary <- data.frame(race = rs, date = ds, n_cases = nc, n_deaths = nd)
write.csv(daily_cases_deaths_31to70_summary,paste(pull_date,"Daily Cases and Deaths by Race (test or death Date).csv",sep=" - "))


## By Region

tested$RR[tested$EMS %in% c(7:11)] <- "Northeast"
...
daily_cases_deaths_31to70 <- subset(tested,!(tested$age_group %in% c("Under 21","21-30","71-80","Over 80")) & tested$age_group!="" )
daily_cases_deaths_31to70 <- daily_cases_deaths_31to70[,c("id","RR","specimen_collection","deceased_date","test_race_label","death_race_label")]
...
for(i in 1:length(dates))
{
   for (r in levels(daily_cases_deaths_31to70$RR))
   {
      sub <- subset(daily_cases_deaths_31to70,paste(daily_cases_deaths_31to70$RR) == r)
...
daily_cases_deaths_31to70_summary <- data.frame(region = rs, date = ds, n_cases = nc, n_deaths = nd)
write.csv(daily_cases_deaths_31to70_summary,paste(pull_date,"Daily Cases and Deaths by Region (test date).csv",sep=" - "))
...
daily_cases_deaths_31to70_summary <- data.frame(region = rs, date = ds, n_cases = nc, n_deaths = nd)
write.csv(daily_cases_deaths_31to70_summary,paste(pull_date,"Daily Cases and Deaths by Region (test or death Date).csv",sep=" - "))
```
