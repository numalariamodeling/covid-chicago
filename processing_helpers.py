...
def load_sim_data(exp_name, region_suffix ='_All', input_wdir=None, fname='trajectoriesDat.csv',
    ...
    if select_traces is not False and region_suffix is not None:
        ...
        if os.path.exists(os.path.join(sim_output_path, 'traces_ranked_region_{str(ems_nr)}.csv')):
            rank_export_df = pd.read_csv(os.path.join(sim_output_path, 'traces_ranked_region_{str(ems_nr)}.csv'))
