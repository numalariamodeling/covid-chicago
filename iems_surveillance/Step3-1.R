...
for (i in 1:length(data1[,7])){
  for (j in 1:50){
    downsample1[j]<-rbinom(n=1,size=data1[i,7],prob=15/2000)
    downsample2[j]<-rbinom(n=1,size=data1[i,7],prob=30/2000)
    downsample3[j]<-rbinom(n=1,size=data1[i,7],prob=60/2000)
    downsample4[j]<-rbinom(n=1,size=data1[i,7],prob=150/2000)
    downsample5[j]<-rbinom(n=1,size=data1[i,7],prob=500/2000)
  }
