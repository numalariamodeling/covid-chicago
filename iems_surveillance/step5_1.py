...
def create_mitigation_df(path):
    ...
    if Rt_series[-1]>0.5:
        print('mitigation')
        mitigation_date = DateToTimestep(Dates[300])+4
        ...
        mitigation_df = pd.DataFrame(mitigation, columns = ['Date', 'ki_reset'])
        ...
        mitigation_df.to_csv(os.path.join('../experiment_configs', 'input_csv','sample_parameters_mitigation.csv'),index=False)
    else:
