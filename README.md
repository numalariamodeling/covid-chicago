...
- `0_runTraceSelection.bat`  calls a [Python script](https://github.com/numalariamodeling/covid-chicago/blob/master/plotters/trace_selection.py) calculating the negative log-likelihood per simulated trajectory, used for thinning predictions and parameter estimation
...
- `3_runProcessForCivis.bat`  calls a [Python](https://github.com/numalariamodeling/covid-chicago/blob/master/plotters/estimate_Rt_forCivisOutputs.py)  that  runs the Rt estimation, the Rt columns are added to the result csv dataframe (i.e. nu_20201005.csv), produces descriptive plots
...
- `5_runProcessFor_CDPH.bat`  calls both a [Python](https://github.com/numalariamodeling/covid-chicago/blob/master/data_processing/emresource_cli_per_covidregion.py) and a [Rscript](https://github.com/numalariamodeling/covid-chicago/blob/master/Rfiles/estimate_Rt/covidregion_Rt_timelines.R) that generates region Rt timelines and data trends plot.  
- `5_runProcessForCivis_optional.bat` calls a [Python](https://github.com/numalariamodeling/covid-chicago/blob/master/plotters/iteration_comparison.py) that  generates the iteration comparison plot (last 3 weeks)
